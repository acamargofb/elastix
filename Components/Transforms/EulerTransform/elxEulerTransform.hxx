                                                              ...
                                                              ParameterMapType *     paramsMap) const
{
  ...
  /** Get the center of rotation point and write it to file. */
  paramsMap->insert({ "CenterOfRotationPoint", BaseComponent::ToVectorOfStrings(m_EulerTransform->GetCenter()) });

  /** Write the ComputeZYX to file. */
  if (SpaceDimension == 3)
  {
    paramsMap->insert({ "ComputeZYX", { BaseComponent::ToString(m_EulerTransform->GetComputeZYX()) } });
  }
